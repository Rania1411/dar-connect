...
export default function HouseCard({ house, userId }) {
  ...
  const handleBooking = async (e) => {
    e.preventDefault()
    if (!date || !file) {
      setError('Please select a date and upload your ID scan.')
      return
    }
    ...
    <>
      <div className="bg-dark-card rounded-2xl overflow-hidden border border-dark-border hover:border-primary/40 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-primary/10 group">
        <div className="relative overflow-hidden h-52">
          <img
            src={house.image_url || `https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=800&q=80`}
            alt={house.title}
        ...
        <div className="p-5">
          <h3 className="font-display font-semibold text-white text-lg leading-snug mb-1">
            {house.title}
          </h3>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="bg-primary hover:bg-primary-dark text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-all duration-200 hover:shadow-lg hover:shadow-primary/30"
            >
              Book Visit
            </button>
          </div>
            ...
            {success ? (
              ...
              <form onSubmit={handleBooking} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-primary hover:bg-primary-dark disabled:opacity-60 text-white font-semibold py-3 rounded-xl transition-all duration-200 hover:shadow-lg hover:shadow-primary/30 flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <>
